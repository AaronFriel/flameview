...
# Skip full setup if it has already completed. This avoids repeatedly
# installing toolchains and packages when `.agent/hooks/pre-push.sh` runs.
if [ -f .agent/.setup_done ]; then
...
# Precompile the workspace so that later runs of `.agent/hooks/pre-push.sh`
# are incremental and complete faster.
cargo fetch
cargo build --workspace --release --exclude flameview-fuzz
cargo test --workspace --all-features --no-run

touch .agent/.setup_done
